...
| Checked-exception-like subset | partial | JAVA2GO supports only a narrow exception-flow subset: local validation throws, single-operation fallback, simple retry loops, and parse-failure catch-to-error-return. Broad checked exception typing and propagation remain unsupported. | Review fallback semantics, retry behavior, and explicit Go error handling before trusting converted control flow. |
