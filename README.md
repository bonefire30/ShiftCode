...
- 分层评测套件（`smoke` / `core` / `features` / `wave1`）和项目级状态分类说明。
- wave1 项目级 explainability：可以解释为什么整体结果是 `partial`，并指出需要优先 review 的模块。

当前质量证据：

- 单元测试：`python -m unittest discover -s tests`，52 个测试通过。
- Python 编译检查覆盖核心后端文件和 smoke 脚本。
- 最终 smoke 样例：`run_logs/tier5_three_profile_smoke_20260426_163606.json`。
- 最新 wave1 explainability 样例：`run_logs/layered_eval_wave1_codex_proxy_after_project_summary_v2.json`。
...
外部试用入口文档：`docs/external-trial-guide.md`
...
For the intended role of `benchmark_dataset/`, `benchmark_projects/`, and
`examples/`, see `docs/test-assets-structure.md`.
...
- Optional MCP and skill integrations are still supported by the project-level
  agents.
- For small external evaluation packaging, use `docs/external-trial-guide.md`,
  `docs/external-trial-checklist.md`, and
